...
BOOL LoadLanguageFile ()
{
	...
	int headers[] = { IDR_COMMON_RSRC_HEADER, 128, 0 };
			...
			while (xml = XmlFindElement (xml, xmlElements[i]))
			{
					...
					|| strcmp (attr, langId) == 0)
				{
					if (XmlGetAttributeText (xml, "key", attr, sizeof (attr)))
					{
						...
						// Parse \ escape sequences
						{
							...
							while (*in)
							{
								if (*in == '\\')
								{
									...
									default:
										MessageBoxA (0, key, "TrueCrypt: Unknown '\\' escape sequence in string", MB_ICONERROR);
										return FALSE;
									}
								}
								else
						...
						{
							MessageBoxA (0, key, "TrueCrypt: Error while decoding UTF-8 string", MB_ICONERROR);
							return FALSE;
						}
						...
						&& (!defaultLangFound || wcscmp (wattr, L"English") != 0))
					{
						...
						if (i >= 0)
						{
							...
							if (strcmp (attr, PreferredLangId) == 0)
							{
								...
								{
									swprintf (szVers, 200,  GetString("LANG_PACK_VERSION"), L"--");
								}
								else
								{
									...
									if (nLen != 0 && nLen != ERROR_NO_UNICODE_TRANSLATION)
										swprintf (szVers, 200, GetString("LANG_PACK_VERSION"), wversion);
								}
