...
class CWebBrowserCtrl: 
	public CWebBrowser2<CWebBrowserCtrl>,
	public CWindowImpl<CWebBrowserCtrl, CAxWindow>
{
	typedef CWebBrowserCtrl thisClass;
	typedef CWindowImpl<CWebBrowserCtrl, CAxWindow> baseClass;

public:
	//enum { IDD = IDD_FIND_SHARINGS };
	...
	BEGIN_MSG_MAP(CWebBrowserCtrl)
		CHAIN_MSG_MAP(CWebBrowser2<CWebBrowserCtrl>)
	...
	LRESULT OnWindowPosChanged(UINT, WPARAM, LPARAM, BOOL& bHandled )
	{
		...
		::MoveWindow( *this, rc.left, rc.top, rc.right, rc.bottom, TRUE );
		bHandled = FALSE;
		return 0;
	}	
