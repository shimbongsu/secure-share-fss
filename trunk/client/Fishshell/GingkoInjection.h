#ifndef __GINGKO_INJECTION_H__
#define __GINGKO_INJECTION_H__
...
typedef struct _MODREF {
    struct _MODREF*  pNextModRef;    //00 Pointer to next MODREF in list (EOL=NULL)
...
typedef HMODULE (WINAPI *LOADLIBRARY)(TCHAR *); // ANSI
typedef BOOL (WINAPI *FREELIBRARY)(HMODULE);
typedef HMODULE (WINAPI *GETMODULEHANDLE)(TCHAR *); // ANSI
// System functions loaded dinamically
...
typedef struct {
    ...
    TCHAR           szDllPath[MAX_PATH + 1];	// Name of Dll
...
extern LDRSHUTDOWNTHREAD           LdrShutdownThread;
extern RTLNTSTATUSTODOSERROR       RtlNtStatusToDosError;
extern RTLCREATEUSERTHREAD         RtlCreateUserThread;
extern NTALLOCATEVIRTUALMEMORY     NtAllocateVirtualMemory;
extern NTFREEVIRTUALMEMORY         NtFreeVirtualMemory;
extern NTOPENTHREAD                NtOpenThread;
...
extern NTQUEUEAPCTHREAD            NtQueueApcThread;
extern NTTERMINATETHREAD           NtTerminateThread;
extern LPFN_ISWOW64PROCESS				   _IsWow64Process;
...
int IsCodeSafe(PBYTE pCode, DWORD *CodeLen);
