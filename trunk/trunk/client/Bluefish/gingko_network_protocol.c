...
--*/
{
    ...
    do
    {
        ...
        GingkoNdisReferenceAdapt(pAdapt);
...
#pragma prefast(suppress: __WARNING_POTENTIAL_BUFFER_OVERFLOW, "Ndis guarantees MediumIndex to be within bounds");
        ...
        *Status = NdisIMInitializeDeviceInstanceEx(DriverHandle,
        ...
        if (*Status != NDIS_STATUS_SUCCESS)
        {
            ...
            if (GingkoNdisDereferenceAdapt(pAdapt))
            {
                pAdapt = NULL;
            }
        ...
        GingkoNdisDereferenceAdapt(pAdapt);
...
--*/
{
    ...
    if (pAdapt->QueuedRequest == TRUE)
    {
        ...
        GingkoNdisRequestComplete(pAdapt,
                         &pAdapt->Request,
                         NDIS_STATUS_FAILURE );
    ...
    else
    {
        ...
        //
        GingkoNdisFreeAllPacketPools(pAdapt);
...
VOID
GingkoNdisCloseAdapterComplete(
    IN    NDIS_HANDLE            ProtocolBindingContext,
    IN    NDIS_STATUS            Status
    )
/*++
...
--*/
{
    ...
    switch (NdisRequest->RequestType)
    {
      case NdisRequestQueryInformation:
        ...
        {
            GingkoNdisQueryPNPCapabilities(pAdapt, &Status);
        }
...
VOID
GingkoNdisTransferDataComplete(
    IN  NDIS_HANDLE         ProtocolBindingContext,
    IN  PNDIS_PACKET        Packet,
    IN  NDIS_STATUS         Status,
...
--*/
{
    ...
    switch (pNetPnPEvent->NetEvent)
    {
        case NetEventSetPower:
            Status = GingkoNdisPnPNetEventSetPower(pAdapt, pNetPnPEvent);
            break;

         case NetEventReconfigure:
            Status = GingkoNdisPnPNetEventReconfigure(pAdapt, pNetPnPEvent);
            break;
...
NDIS_STATUS
GingkoNdisPnPNetEventReconfigure(
    IN PADAPT            pAdapt,
    IN PNET_PNP_EVENT    pNetPnPEvent
    )
/*++
...
--*/
{
    ...
    if (pAdapt->PTDeviceState > NdisDeviceStateD0)
    {
        ...
        if (pAdapt->QueuedRequest)
        {
            pAdapt->QueuedRequest = FALSE;
            NdisReleaseSpinLock(&pAdapt->Lock);
            GingkoNdisRequestComplete(pAdapt, &pAdapt->Request, NDIS_STATUS_FAILURE);
        }
        else
        {
            NdisReleaseSpinLock(&pAdapt->Lock);
