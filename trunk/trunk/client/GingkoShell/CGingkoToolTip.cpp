...
/*
struct GINGKO_TOOLTIP_ENUM_CHILD 
{
	HWND hwndExclude;
	HWND hWnd;
	POINT ptScreen;
...
{ 
	GINGKO_TOOLTIP_ENUM_CHILD * structEnum = (GINGKO_TOOLTIP_ENUM_CHILD*)lParam;
...
CGingkoToolTip::CGingkoToolTip()
{
	...
	m_dwTimeFadeIn = 500;
	m_dwTimeFadeOut = 500;
	m_dwBehaviour = 0; //GINGKO_TOOLTIP_CLOSE_LEAVEWND | GINGKO_TOOLTIP_NOCLOSE_OVER;	 //The tooltip's behaviour
	m_dwEffectBk = 0;
	m_dwDirection = 0;
	m_dwStyles = 0;
	m_nGranularity = 0;
	...
	m_nTooltipState = GINGKO_TOOLTIP_STATE_HIDEN;
	m_nTooltipType = GINGKO_TOOLTIP_NORMAL;
	m_nNextTooltipType = GINGKO_TOOLTIP_NORMAL;
...
//	EnableTextWrap(FALSE);
	SetDelayTime(GINGKO_TOOLTIP_TIME_INITIAL, 500);
	SetDelayTime(GINGKO_TOOLTIP_TIME_AUTOPOP, 5000);
	SetDelayTime(GINGKO_TOOLTIP_TIME_FADEIN, 0);
	SetDelayTime(GINGKO_TOOLTIP_TIME_FADEOUT, 0);
	SetTooltipShadow(6, 6);
	
	// Register the window class if it has not already been registered.
	//WNDCLASS wndcls;
	//HINSTANCE hInst = AfxGetInstanceHandle();
	//if(!(::GetClassInfo(hInst, GINGKO_TOOLTIP_CLASSNAME, &wndcls)))
	//{
	...
	//	wndcls.lpszClassName	= GINGKO_TOOLTIP_CLASSNAME;
	//	
...
//BEGIN_MESSAGE_MAP(CGingkoToolTip, CWnd)
...
BOOL CGingkoToolTip::Create(HWND  pParentWnd, BOOL bBalloon /* = TRUE */) 
{
	TRACE(_T("CGingkoToolTip::Create\n"));
	
	ATLASSERT(pParentWnd != NULL);
	
	DWORD dwStyle = WS_POPUP; 
	DWORD dwExStyle = WS_EX_TOOLWINDOW | WS_EX_TOPMOST;
	m_hParentWnd = pParentWnd;
	
	if (!CreateEx(dwExStyle, GINGKO_TOOLTIP_CLASSNAME, NULL, dwStyle, 0, 0, 0, 0, pParentWnd, NULL, NULL))
		return FALSE;

	//
	SetDefaultSizes(bBalloon);
	//m_drawer.SetCallbackRepaint(*this, UDM_TOOLTIP_REPAINT);
	SetDelayTime(GINGKO_TOOLTIP_TIME_ANIMATION, 100);

	return TRUE;
} //End of Create

BOOL CGingkoToolTip::DestroyWindow() 
{
	Pop();
	SetDelayTime(GINGKO_TOOLTIP_TIME_ANIMATION, 0);
	return CWindow::DestroyWindow();
} //End of DestroyWindow

/////////////////////////////////////////////////////////////////////
//		A tooltip with GINGKO_TOOLTIP_DISABLE_AUTOPOP behaviour don't hide on 
//	change active application
//-------------------------------------------------------------------
// Fixed by vanhoopy (July 10, 2003)
/////////////////////////////////////////////////////////////////////
#if _MSC_VER < 1300
void CGingkoToolTip::OnActivateApp(BOOL bActive, HTASK hTask)
#else
void CGingkoToolTip::OnActivateApp(BOOL bActive, DWORD hTask)
#endif //_MSC_VER
{
...
LRESULT CGingkoToolTip::SendNotify(LPPOINT pt, GINGKO_TOOLTIP_INFO & ti) 
{ 
	TRACE(_T("CGingkoToolTip::SendNotify()\n")); 
	...
	NM_GINGKO_TOOLTIP_DISPLAY lpnm; 
...
///////////////////////////////////////////////////////////////////////
BOOL CGingkoToolTip::IsNotify()
{
	TRACE(_T("CGingkoToolTip::IsNotify\n"));
	
	return (BOOL)(m_hNotifyWnd != NULL);
}  //End of IsNotify

BOOL CGingkoToolTip::OnSetCursor(HWND  pWnd, UINT nHitTest, UINT message) 
{
	CPoint ptClient;
	::GetCursorPos(&ptClient);
	ScreenToClient(&ptClient);
	TRACE (_T("CGingkoToolTip::OnSetCursor(x=%d, y=%d)\n"), ptClient.x, ptClient.y);
...
LRESULT CGingkoToolTip::OnRepaintWindow(WPARAM wParam, LPARAM lParam)
{
	TRACE (_T("CGingkoToolTip::OnRepaintWindow()\n"));
...
void CGingkoToolTip::OnDrawBorder(HDC hDC, HRGN hRgn)
{
	ATLASSERT (hDC);
	ATLASSERT (hRgn);
...
//
////////////////////////////////////////////////////////////////////////
void CGingkoToolTip::OnRedrawTooltip(HDC hDC, BYTE nTransparency /* = 0 */)
{
	TRACE (_T("CGingkoToolTip::OnRedrawTooltip(Transparency = %d)\n"), nTransparency);
...
void CGingkoToolTip::OnPaint() 
{
	TRACE(_T("CGingkoToolTip::OnPaint()\n"));
...
BOOL CGingkoToolTip::PreTranslateMessage(MSG* pMsg) 
{
	RelayEvent(pMsg);
...
BOOL CGingkoToolTip::RelayEvent(MSG* pMsg)
{
	...
	case WM_SETFOCUS:
		...
		break;
	case WM_LBUTTONDOWN:
		TRACE(_T("CGingkoToolTip::WM_LBUTTONDOWN\n"));
...
//		if (!(m_tiDisplayed.nBehaviour & GINGKO_TOOLTIP_NOCLOSE_MOUSEDOWN))
		Pop();
		break;
	case WM_MOUSEMOVE:
		if ((GINGKO_TOOLTIP_HELP == m_nTooltipType) || (GINGKO_TOOLTIP_HELP == m_nNextTooltipType))
			return FALSE;
		if ((m_ptOriginal.x != pMsg->pt.x) || (m_ptOriginal.y != pMsg->pt.y))
		{
			...
			else if (IsCursorOverTooltip() && !(m_tiDisplayed.nBehaviour & GINGKO_TOOLTIP_TRACKING_MOUSE)) 
			{
				//ENG: Mouse over a tooltip and tracking mode was disabled
				if (!(m_tiDisplayed.nBehaviour & GINGKO_TOOLTIP_NOCLOSE_OVER))
				{
			...
			else
			{
				...
				else 
				{
					...
					else
					{
						...
						if (m_tiDisplayed.nBehaviour & GINGKO_TOOLTIP_TRACKING_MOUSE)
						{
							//ENG: If sets tracking mode
							SetAutoPopTimer();
							OutputTooltipOnScreen(&pMsg->pt);
						}
						else if (!(m_tiDisplayed.nBehaviour & GINGKO_TOOLTIP_CLOSE_LEAVEWND))
						{
							//ENG: A tooltip must hide at anything mouse move
							if ((hWnd == m_hwndDisplayedTool) && 
								!(m_tiDisplayed.nBehaviour & GINGKO_TOOLTIP_MULTIPLE_SHOW))
							{
...
void CGingkoToolTip::SetNewTooltip(HWND hWnd, const GINGKO_TOOLTIP_INFO & ti, BOOL bDisplayWithDelay /* = TRUE */, TooltipType type /* = GINGKO_TOOLTIP_NORMAL */)
{
	TRACE (_T("CGingkoToolTip::SetNewTooltip(hWnd=0x%08X, CRect(left=%d, top=%d, right=%d, bottom=%d), nID=%d)\n"), 
		hWnd, ti.rectBounds.left, ti.rectBounds.top, ti.rectBounds.right, ti.rectBounds.bottom, ti.nIDTool);
	
	m_bNextToolExist = FALSE;

	//ENG: Hides a tooltip
	if ((GINGKO_TOOLTIP_STATE_SHOWING == m_nTooltipState) || 
		(GINGKO_TOOLTIP_STATE_SHOWN == m_nTooltipState))
		HideTooltip();
	...
	m_hwndNextTool = hWnd;
	m_tiNextTool = ti;
	if (GINGKO_TOOLTIP_STATE_HIDEN != m_nTooltipState)
	{
...
void CGingkoToolTip::OnTimer(UINT nIDEvent) 
{
	POINT pt;
	switch (nIDEvent)
	{
	case TIMER_SHOW:
		...
		if ((GINGKO_TOOLTIP_HELP == m_nNextTooltipType) || 
			(GINGKO_TOOLTIP_MENU == m_nNextTooltipType))
			pt = m_ptOriginal;
		...
		else if (GINGKO_TOOLTIP_STATE_HIDEN == m_nTooltipState)
		{
			TRACE(_T("OnTimerShow(Showing)\n"));
			m_nTooltipState = GINGKO_TOOLTIP_STATE_SHOWING;
			//Display first step
			...
			else
			{
				...
				m_nTooltipState = GINGKO_TOOLTIP_STATE_SHOWN; //Tooltip is already show
				if (m_dwTimeAutoPop && !(m_tiNextTool.nBehaviour & GINGKO_TOOLTIP_DISABLE_AUTOPOP))
	...
	case TIMER_SHOWING:
		...
		if (m_dwCurTransparency <= m_tiDisplayed.nTransparency)
		{
			...
			m_nTooltipState = GINGKO_TOOLTIP_STATE_SHOWN;
	...
	case TIMER_HIDE:
		...
		if (GINGKO_TOOLTIP_STATE_HIDING != m_nTooltipState)
		{
			m_nTooltipState = GINGKO_TOOLTIP_STATE_HIDING;
	...
	case TIMER_HIDING:
		...
		if (PERCENT_MAX_TRANSPARENCY == m_dwCurTransparency)
		{
			...
			if (m_tiDisplayed.nBehaviour & GINGKO_TOOLTIP_MULTIPLE_SHOW)
			{
...
//				m_nNextTooltipType = GINGKO_TOOLTIP_NORMAL;
	...
	case TIMER_ANIMATION:
		if (IsVisible() && (GINGKO_TOOLTIP_STATE_SHOWN == m_nTooltipState))
		{
			//if(m_drawer.OnTimer())
			//	OnRedrawTooltip(NULL, m_dwCurTransparency);
		} //if
...
void CGingkoToolTip::HideTooltip()
{
	TRACE (_T("CGingkoToolTip::HideTooltip(CurState=%d)\n"), m_nTooltipState);
	switch(m_nTooltipState)
	{
	case GINGKO_TOOLTIP_STATE_SHOWING:
		//ENG: Kill showing tooltip
		KillTimer(TIMER_SHOWING);
	case GINGKO_TOOLTIP_STATE_SHOWN:
...
void CGingkoToolTip::SetAutoPopTimer()
{
	TRACE (_T("CGingkoToolTip::SetAutoPopTimer()\n"));
	if (m_dwTimeAutoPop && !(m_tiDisplayed.nBehaviour & GINGKO_TOOLTIP_DISABLE_AUTOPOP))
		SetTimer(TIMER_HIDE, m_dwTimeAutoPop, NULL);
} //End of SetAutoPopTimer

void CGingkoToolTip::KillTimers(DWORD dwIdTimer /* = NULL */)
{
	TRACE (_T("CGingkoToolTip::KillTimers()\n"));
	if (dwIdTimer == NULL)
	{
		KillTimer(TIMER_SHOW);
		KillTimer(TIMER_HIDE);
...
void CGingkoToolTip::Pop()
{
	TRACE (_T("CGingkoToolTip::Pop()\n"));
	m_nTooltipState = GINGKO_TOOLTIP_STATE_HIDEN;
	m_nTooltipType = GINGKO_TOOLTIP_NORMAL;
	m_nNextTooltipType = GINGKO_TOOLTIP_NORMAL;
	KillTimers();
...
void CGingkoToolTip::PrepareDisplayTooltip(LPPOINT lpPoint)
{
	TRACE (_T("CGingkoToolTip::PrepareDisplayTooltip()\n"));
	
	//Fills default members
	if (!(m_tiNextTool.nMask & GINGKO_TOOLTIP_MASK_STYLES))
		m_tiNextTool.nStyles = m_dwStyles;

	if (!(m_tiNextTool.nMask & GINGKO_TOOLTIP_MASK_EFFECT))
	{
		m_tiNextTool.nEffect = m_dwEffectBk;
		m_tiNextTool.nGranularity = m_nGranularity;
	} //if

	if (!(m_tiNextTool.nMask & GINGKO_TOOLTIP_MASK_COLORS))
	{
	...
	if (!(m_tiNextTool.nMask & GINGKO_TOOLTIP_MASK_DIRECTION))
		m_tiNextTool.nDirection = m_dwDirection;

	if (!(m_tiNextTool.nMask & GINGKO_TOOLTIP_MASK_BEHAVIOUR))
		m_tiNextTool.nBehaviour = m_dwBehaviour;

	if (!(m_tiNextTool.nMask & GINGKO_TOOLTIP_MASK_TRANSPARENCY))
	...
	m_hBitmapBk = ::CreateCompatibleBitmap(pDC->m_hDC, rect.Width(), rect.Height());
	...
	{
	case GINGKO_TOOLTIP_LEFTEDGE_TOP:
	case GINGKO_TOOLTIP_LEFTEDGE_VCENTER:
	case GINGKO_TOOLTIP_LEFTEDGE_BOTTOM:
		...
		break;
	case GINGKO_TOOLTIP_RIGHTEDGE_TOP:
	case GINGKO_TOOLTIP_RIGHTEDGE_VCENTER:
	case GINGKO_TOOLTIP_RIGHTEDGE_BOTTOM:
		...
		break;
	case GINGKO_TOOLTIP_BOTTOMEDGE_LEFT:
	case GINGKO_TOOLTIP_BOTTOMEDGE_CENTER:
	case GINGKO_TOOLTIP_BOTTOMEDGE_RIGHT:
		...
		break;
	case GINGKO_TOOLTIP_TOPEDGE_LEFT:
	case GINGKO_TOOLTIP_TOPEDGE_CENTER:
	case GINGKO_TOOLTIP_TOPEDGE_RIGHT:
	...
	if (GINGKO_TOOLTIP_MENU == m_nTooltipType) 
	{
		SetWindowPos(NULL, 
			rect.left, rect.top,
...
void CGingkoToolTip::FreeResources()
{
...
void CGingkoToolTip::OutputTooltipOnScreen(LPPOINT lpPoint, HDC hDC /* = NULL */)
{
...
////////////////////////////////////////////////////////////////////
// CGingkoToolTip::GetTooltipDirection()
...
//
////////////////////////////////////////////////////////////////////
DWORD CGingkoToolTip::GetTooltipDirection(DWORD dwDirection, const CPoint & ptPoint, CPoint & ptAnchor, CRect & rcBody, CRect & rcFull, CRect & rcTipArea)
{
...
HRGN CGingkoToolTip::GetTooltipRgn(DWORD dwDirection, int x, int y, int nWidth, int nHeight)
{
	HRGN hRgn = NULL;
...
BOOL CGingkoToolTip::IsCursorOverTooltip() const
{
    ATLASSERT(m_hParentWnd);
	...
	CGingkoToolTip * pWnd = (CGingkoToolTip*)WindowFromPoint(pt);
	
	return (pWnd == this);
}

HWND CGingkoToolTip::GetWndFromPoint(const LPPOINT lpPoint, GINGKO_TOOLTIP_INFO & ti, BOOL bCheckTool /* = TRUE */)
{
...
BOOL CGingkoToolTip::IsComboBoxControl(HWND hWnd, UINT nStyle)
{
	if (hWnd == NULL)
		return FALSE;
...
CString CGingkoToolTip::GetDebugInfoTool(LPPOINT lpPoint)
{
	GINGKO_TOOLTIP_INFO ti;
...
CString CGingkoToolTip::CreateDebugCell(CString sTitle, LPCTSTR lpszDescription)
{
	CString str;
...
CString CGingkoToolTip::GetMaxDebugString(LPCTSTR lpszText)
{
...
HWND CGingkoToolTip::FindTool(const LPPOINT lpPoint, GINGKO_TOOLTIP_INFO & ti)
{
	return GetWndFromPoint(lpPoint, ti, TRUE);
} //End of FindTool

BOOL CGingkoToolTip::FindTool(HWND hWnd, const LPPOINT lpPoint, GINGKO_TOOLTIP_INFO & ti)
{
...
HWND CGingkoToolTip::FindToolBarItem(POINT point, GINGKO_TOOLTIP_INFO & ti)
{
...
//		ti				- Reference to GINGKO_TOOLTIP_INFO structure containing the parameters of the tooltip 
//
// Remarks:
...
////////////////////////////////////////////////////////////////////
void CGingkoToolTip::AddTool(HWND pWnd , DWORD dwIdString, HICON hIcon, LPCRECT lpRectBounds /*= NULL*/, DWORD dwIDTool /*= 0*/)
{
	CString str;
...
void CGingkoToolTip::AddTool(HWND pWnd, LPCTSTR lpszString, HICON hIcon, LPCRECT lpRectBounds /*= NULL*/, DWORD dwIDTool /*= 0*/)
{
	CString str;
...
void CGingkoToolTip::AddTool(HWND pWnd , DWORD dwIdString, DWORD dwIdIcon, CSize & szIcon /* = CSize(0, 0) */, LPCRECT lpRectBounds /*= NULL*/, DWORD dwIDTool /*= 0*/)
{
	CString str;
	str.LoadString(dwIdString);
...
void CGingkoToolTip::AddTool(HWND pWnd , LPCTSTR lpszString, DWORD dwIdIcon, CSize & szIcon /* = CSize(0, 0) */, LPCRECT lpRectBounds /*= NULL*/, DWORD dwIDTool /*= 0*/)
{
	CString str;
...
void CGingkoToolTip::AddTool(HWND pWnd , LPCTSTR lpszString, DWORD dwIdBitmap, COLORREF crMask, CSize & szBitmap /*= CSize(0, 0)*/, LPCRECT lpRectBounds /*= NULL*/, DWORD dwIDTool /*= 0*/)
{
	CString str;
...
void CGingkoToolTip::AddTool(HWND pWnd , DWORD dwIdString, LPCRECT lpRectBounds /* = NULL */, DWORD dwIDTool /* = 0 */)
{
	CString str;
	str.LoadString(dwIdString);
...
void CGingkoToolTip::AddTool(HWND pWnd , LPCTSTR lpszString /* = NULL */, LPCRECT lpRectBounds /* = NULL */, DWORD dwIDTool /* = 0 */)
{
	GINGKO_TOOLTIP_INFO ti;
...
void CGingkoToolTip::AddTool(HWND pWnd , GINGKO_TOOLTIP_INFO & ti)
{
	TRACE(_T("CGingkoToolTip::AddTool(hWnd=0x%08X)\n"), pWnd->GetSafeHwnd());
	ATLASSERT (pWnd);
...
//					      NULL if bounding rectangle don't uses for specified window
////////////////////////////////////////////////////////////////////
void CGingkoToolTip::RemoveTool(HWND pWnd , LPCRECT lpRectBounds /* = NULL */)
{
	TRACE (_T("CGingkoToolTip::RemoveTool(hWnd=0x%08X)\n"), pWnd->GetSafeHwnd());
	ATLASSERT(pWnd);
...
////////////////////////////////////////////////////////////////////
// CGingkoToolTip::RemoveAllTools()
//   Removes all tools from the collection of tools supported by a tooltip control.
////////////////////////////////////////////////////////////////////
void CGingkoToolTip::RemoveAllTools()
{
	TRACE (_T("CGingkoToolTip::RemoveAllTools()\n"));
...
BOOL CGingkoToolTip::GetToolInfo(GINGKO_TOOLTIP_INFO & ti, HWND pWnd , LPCRECT lpRectBounds /* = NULL */)
{
	ATLASSERT(pWnd);
...
BOOL CGingkoToolTip::GetToolInfo(GINGKO_TOOLTIP_INFO & ti, HWND pWnd , DWORD dwIDTool /* = 0 */)
{
	ATLASSERT(pWnd);
...
void CGingkoToolTip::UpdateTipText(LPCTSTR lpszText, HWND pWnd , DWORD dwIDTool /* = 0 */)
{
	GINGKO_TOOLTIP_INFO ti;
...
void CGingkoToolTip::DelTool(HWND pWnd , DWORD dwIDTool)
{
	GINGKO_TOOLTIP_INFO ti;
	if (GetToolInfo(ti, pWnd, dwIDTool))
	{
...
void CGingkoToolTip::SetToolRect(HWND pWnd , DWORD dwIDTool, LPCRECT lpRectBounds)
{
	GINGKO_TOOLTIP_INFO ti;
	if (GetToolInfo(ti, pWnd, dwIDTool))
	{
...
////////////////////////////////////////////////////////////////////
// CGingkoToolTip::EnableHyperlink()
// Enables redrawing hyperlinks and hot areas.
////////////////////////////////////////////////////////////////////
void CGingkoToolTip::EnableHyperlink(BOOL bEnable /* = TRUE */)
{
	m_bHyperlinkEnabled = bEnable;
} //End of EnableHyperlink

////////////////////////////////////////////////////////////////////
// CGingkoToolTip::SetCallbackHyperlink()
...
////////////////////////////////////////////////////////////////////
void CGingkoToolTip::SetCallbackHyperlink(HWND hWnd, UINT nMessage, LPARAM lParam /* = 0 */)
{
	TRACE(_T("CGingkoToolTip::SetCallbackHyperlink()\n"));
	
	//m_drawer.SetCallbackHyperlink(hWnd, nMessage, lParam);
} //End of SetCallbackHyperlink

/////////////////////////////////////////////////////////////////////
// CGingkoToolTip::SetNotify()
...
//						  Else the notification will not send
///////////////////////////////////////////////////////////////////////
void CGingkoToolTip::SetNotify(BOOL bParentNotify /* = TRUE */)
{
	HWND hWnd = NULL;
...
void CGingkoToolTip::SetNotify(HWND hWnd)
{
	TRACE(_T("CGingkoToolTip::SetNotify\n"));
	
	m_hNotifyWnd = hWnd;
} //End of SetNotify

/////////////////////////////////////////////////////////////////////////////
//  CGingkoToolTip::SetSize()
...
//		nValue			- size's value
/////////////////////////////////////////////////////////////////////////////
void CGingkoToolTip::SetSize(int nSizeIndex, int nValue)
{
	TRACE(_T("CGingkoToolTip::SetSize(nSizeIndex = %d, nValue = %d)\n"), nSizeIndex, nValue);
...
//---------------------------------------------------------------------------
//  Parameters :
//		nSizeIndex		- An index of the sizes. See CGingkoToolTip::SetSize for a 
//						  description of the valid values.
//  Returns :
//		size's value
//
/////////////////////////////////////////////////////////////////////////////
int CGingkoToolTip::GetSize(int nSizeIndex)
{
	TRACE(_T("CGingkoToolTip::GetSize(nSizeIndex = %d)\n"), nSizeIndex);
	if (nSizeIndex >= PPTTSZ_MAX_SIZES)
		return 0;
...
/////////////////////////////////////////////////////////////////////////////
//  CGingkoToolTip::SetDefaultSizes()
...
/////////////////////////////////////////////////////////////////////////////
void CGingkoToolTip::SetDefaultSizes(BOOL bBalloonSize /* = TRUE */)
{
	TRACE(_T("CGingkoToolTip::SetDefaultSizes()\n"));
...
/////////////////////////////////////////////////////////////////////////////
void CGingkoToolTip::SetColorBk(COLORREF color)
{
	SetColorBk(color, color, color);
} //End of SetColorBk

void CGingkoToolTip::SetColorBk(COLORREF clrBegin, COLORREF clrEnd)
{
	SetColorBk(clrBegin, clrBegin, clrEnd);
} //End of SetColorBk

void CGingkoToolTip::SetColorBk(COLORREF clrBegin, COLORREF clrMid, COLORREF clrEnd)
{
...
/////////////////////////////////////////////////////////////////////////////
void CGingkoToolTip::SetEffectBk(DWORD dwEffect, BYTE nGranularity /* = 5 */)
{
...
//							GINGKO_TOOLTIP_MULTIPLE_SHOW		- Multiple show for single control
//							GINGKO_TOOLTIP_TRACKING_MOUSE	- Tracking for mouse
//							GINGKO_TOOLTIP_CLOSE_LEAVEWND	- Close tooltip if mouse leave the control
//							GINGKO_TOOLTIP_NOCLOSE_OVER		- No close tooltip if mouse over him
//							GINGKO_TOOLTIP_DISABLE_AUTOPOP	- Disables autopop tooltip from timer
/////////////////////////////////////////////////////////////////////////////
void CGingkoToolTip::SetBehaviour(DWORD dwBehaviour /* = 0 */)
{
	m_dwBehaviour = dwBehaviour;
} //End of SetBehaviour

/////////////////////////////////////////////////////////////////////////////
// CGingkoToolTip::GetBehaviour()
//   Gets a tooltip's behaviour 
//---------------------------------------------------------------------------
// Return value:
//		A tooltip's behaviour. See CGingkoToolTip::SetBehaviour for a description of the 
//	valid values.
/////////////////////////////////////////////////////////////////////////////
DWORD CGingkoToolTip::GetBehaviour()
{
	return m_dwBehaviour;
} //End of GetBehaviour

/////////////////////////////////////////////////////////////////////
//  CGingkoToolTip::SetDelayTime()
...
//							GINGKO_TOOLTIP_TIME_AUTOPOP  - Retrieve the length of time the tooltip 
//													  window remains visible if the pointer is 
//													  stationary within a tool's bounding rectangle. 
//							GINGKO_TOOLTIP_TIME_INITIAL  - Retrieve the length of time the pointer 
//			 										  must remain stationary within a tool's bounding 
//													  rectangle before the tool tip window appears. 
//							GINGKO_TOOLTIP_TIME_FADEIN	- Retrieve the length of time for each step of
//													  fade-in effect
//							GINGKO_TOOLTIP_TIME_FADEOUT	- Retrieve the length of time for each step of
//													  fade-out effect
//							GINGKO_TOOLTIP_TIME_ANIMATION  Retrieve the speed for the animation
//						  For compatibility with 1.x versions of CPPToolTip a following values
//						  are available also:
//							TTDT_AUTOPOP			- Same GINGKO_TOOLTIP_TIME_AUTOPOP 
//							TTDT_INITIAL			- Same GINGKO_TOOLTIP_TIME_INITIAL 
//		nTime			- The specified delay time, in milliseconds.
/////////////////////////////////////////////////////////////////////
void CGingkoToolTip::SetDelayTime(DWORD dwDuration, DWORD dwTime)
{
	switch (dwDuration)
	{
	case GINGKO_TOOLTIP_TIME_AUTOPOP:
		m_dwTimeAutoPop = dwTime;
		break;
	case GINGKO_TOOLTIP_TIME_INITIAL:
		m_dwTimeInitial = dwTime;
		break;
	case GINGKO_TOOLTIP_TIME_FADEIN:
		m_dwTimeFadeIn = dwTime;
		break;
	case GINGKO_TOOLTIP_TIME_FADEOUT:
		m_dwTimeFadeOut = dwTime;
		break;
	case GINGKO_TOOLTIP_TIME_ANIMATION:
...
/////////////////////////////////////////////////////////////////////
// CGingkoToolTip::GetDelayTime()
// Retrieves the initial, pop-up, and reshow durations currently set 
// for a CPPToolTip control
//-------------------------------------------------------------------
// Parameters:
//		dwDuration		- Flag that specifies which duration value will be retrieved. 
//						  See CGingkoToolTip::SetDelayTime for a description of the valid values. 
// Return value:
//	The specified delay time, in milliseconds
///////////////////////////////////////////////////////////////////////
DWORD CGingkoToolTip::GetDelayTime(DWORD dwDuration) const
{
	DWORD dwTime = 0;
	switch (dwDuration)
	{
...
/////////////////////////////////////////////////////////////////////////////
//  CGingkoToolTip::SetDirection()
//    Sets a placement of the tooltip's anchor
//---------------------------------------------------------------------------
//  Parameters :
//		dwDirection		- A placement of the tooltip's anchor. This parameter 
//					      can be one of the following values:
//							GINGKO_TOOLTIP_TOPEDGE_LEFT			- A left corner of the top edge
//							GINGKO_TOOLTIP_TOPEDGE_RIGHT			- A right corner of the top edge
//							GINGKO_TOOLTIP_TOPEDGE_CENTER		- By center of the top edge
//							GINGKO_TOOLTIP_BOTTOMEDGE_LEFT		- A left corner of the bottom edge
//							GINGKO_TOOLTIP_BOTTOMEDGE_RIGHT		- A right corner of the bottom edge
//							GINGKO_TOOLTIP_BOTTOMEDGE_CENTER		- By center of the bottom edge
//							GINGKO_TOOLTIP_LEFTEDGE_TOP			- A top corner of the left edge
//							GINGKO_TOOLTIP_LEFTEDGE_BOTTOM		- A bottom corner of the left edge
//							GINGKO_TOOLTIP_LEFTEDGE_VCENTER		- By center of the left edge
//							GINGKO_TOOLTIP_RIGHTEDGE_TOP			- A top corner of the right edge
//							GINGKO_TOOLTIP_RIGHTEDGE_BOTTOM		- A bottom corner of the right edge
//							GINGKO_TOOLTIP_RIGHTEDGE_VCENTER		- By center of the right edge
//						  For compatibility with 1.x versions of CPPToolTip a following values
//						  are available also:
//							GINGKO_TOOLTIP_LEFT_TOP				- Same GINGKO_TOOLTIP_TOPEDGE_LEFT
//							GINGKO_TOOLTIP_RIGHT_TOP				- Same GINGKO_TOOLTIP_TOPEDGE_RIGHT
//							GINGKO_TOOLTIP_LEFT_BOTTOM			- Same GINGKO_TOOLTIP_BOTTOMEDGE_LEFT
//							GINGKO_TOOLTIP_RIGHT_BOTTOM			- Same GINGKO_TOOLTIP_BOTTOMEDGE_RIGHT
/////////////////////////////////////////////////////////////////////////////
void CGingkoToolTip::SetDirection(DWORD dwDirection /* = GINGKO_TOOLTIP_BOTTOMEDGE_LEFT */)
{
	TRACE(_T("CGingkoToolTip::SetDirection(nDirection = %d)\n"), dwDirection);

	m_dwDirection = dwDirection;
} //End of SetDirection	

/////////////////////////////////////////////////////////////////////////////
//  CGingkoToolTip::GetDirection()
//    Gets a placement of the tooltip's anchor
//---------------------------------------------------------------------------
//  Returns :
//	  A placement of the tooltip's anchor. See CGingkoToolTip::SetDirection for a description of 
//	the valid values.
/////////////////////////////////////////////////////////////////////////////
DWORD CGingkoToolTip::GetDirection()
{
	TRACE(_T("CGingkoToolTip::GetDirection()\n"));

	return m_dwDirection;
} //End of GetDirection

/////////////////////////////////////////////////////////////////////////////
//  CGingkoToolTip::SetTextStyles()
...
/////////////////////////////////////////////////////////////////////////////
void CGingkoToolTip::SetTextStyle(LPCTSTR lpszStyleName, LPCTSTR lpszStyleValue)
{
	//m_drawer.SetTextStyle(lpszStyleName, lpszStyleValue);
}

/////////////////////////////////////////////////////////////////////////////
//  CGingkoToolTip::SetCssStyles()
...
/////////////////////////////////////////////////////////////////////////////
void CGingkoToolTip::SetCssStyles(LPCTSTR lpszCssStyles /* = NULL */)
{
...
//      lpszPathDll		- 
///////////////////////////////////////////////////////////////////////////// 
void CGingkoToolTip::SetCssStyles(DWORD dwIdCssStyle, LPCTSTR lpszPathDll /* = NULL */) 
{ 
    //m_drawer.SetCssStyles(dwIdCssStyle, lpszPathDll); 
} //End of SetCssStyles

/////////////////////////////////////////////////////////////////////////////
//  CGingkoToolTip::GetCssStyles()
//    Applies a CSS-like styles for the tooltip's HTML
//---------------------------------------------------------------------------
//  Return value:
...
/////////////////////////////////////////////////////////////////////////////
LPCTSTR CGingkoToolTip::GetCssStyles()
{
...
/////////////////////////////////////////////////////////////////////////////
void CGingkoToolTip::SetDebugMode(BOOL bDebug /* = TRUE */)
{
	m_bDebugMode = bDebug;
} //End of SetDebugMode

////////////////////////////////////////////////////////////////////
// CGingkoToolTip::ShowHelpTooltip()
...
//		ti				- Reference to GINGKO_TOOLTIP_INFO structure containing the parameters of the tooltip 
////////////////////////////////////////////////////////////////////
void CGingkoToolTip::ShowHelpTooltip(LPPOINT pt, DWORD dwIdText, HICON hIcon /* = NULL */)
{
	CString str;
...
void CGingkoToolTip::ShowHelpTooltip(LPPOINT pt, DWORD dwIdText, DWORD dwIdIcon, CSize & szIcon /* = CSize(0, 0) */)
{
	CString str;
	str.LoadString(dwIdText);
...
void CGingkoToolTip::ShowHelpTooltip(LPPOINT pt, LPCTSTR lpszString, HICON hIcon /* = NULL */)
{
	GINGKO_TOOLTIP_INFO ti;
	if (NULL == hIcon)
	{
...
void CGingkoToolTip::ShowHelpTooltip(LPPOINT pt, LPCTSTR lpszString, DWORD dwIdIcon, CSize & szIcon /* = CSize(0, 0) */)
{
	CString str;
	str.Format(_T("<table><tr><td><icon idres=%d width=%d height=%d></td><td>%s</td></tr></table>"), 
		dwIdIcon, szIcon.cx, szIcon.cy, lpszString);
...
void CGingkoToolTip::ShowHelpTooltip(LPPOINT pt, GINGKO_TOOLTIP_INFO & ti)
{
	TRACE(_T("CGingkoToolTip::ShowHelpTooltip\n"));

	m_ptOriginal = CPoint(pt->x, pt->y);
	ti.nBehaviour = m_dwBehaviour | GINGKO_TOOLTIP_DISABLE_AUTOPOP;
	ti.nMask = GINGKO_TOOLTIP_MASK_BEHAVIOUR;
	SetNewTooltip(NULL, ti, FALSE, GINGKO_TOOLTIP_HELP);
} //End ShowHelpTooltip

////////////////////////////////////////////////////////////////////
// CGingkoToolTip::SetBorder()
...
////////////////////////////////////////////////////////////////////
void CGingkoToolTip::SetBorder(COLORREF color, int nWidth /* = 1 */, int nHeight /* = 1 */)
{
	HBRUSH hbr = ::CreateSolidBrush(color);
	SetBorder(hbr, nWidth, nHeight);
} //End of SetBorder

void CGingkoToolTip::SetBorder(HBRUSH hbr, int nWidth /* = 1 */, int nHeight /* = 1 */)
{
	HideBorder();
...
////////////////////////////////////////////////////////////////////
// CGingkoToolTip::HideBorder()
// Hides border of the tooltip.
////////////////////////////////////////////////////////////////////
void CGingkoToolTip::HideBorder()
{
	if (NULL != m_hbrBorder)
	{
...
////////////////////////////////////////////////////////////////////
// CGingkoToolTip::EnableEscapeSequences()
...
////////////////////////////////////////////////////////////////////
void CGingkoToolTip::EnableEscapeSequences(BOOL bEnable)
{
	//m_drawer.EnableEscapeSequences(bEnable);
} //End of EnableEscapeSequences

////////////////////////////////////////////////////////////////////
// CGingkoToolTip::SetImageList()
...
////////////////////////////////////////////////////////////////////
void CGingkoToolTip::SetImageList(UINT nIdBitmap, int cx, int cy, int nCount, COLORREF crMask /* = RGB(255, 0, 255) */)
{
	//m_drawer.SetImageList(nIdBitmap, cx, cy, nCount, crMask);
} //End of SetImageList

void CGingkoToolTip::SetImageList(HBITMAP hBitmap, int cx, int cy, int nCount, COLORREF crMask /* = RGB(255, 0, 255) */)
{
	//m_drawer.SetImageList(hBitmap, cx, cy, nCount, crMask);
} //End of SetImageList

////////////////////////////////////////////////////////////////////
// CGingkoToolTip::SetTransparency()
...
////////////////////////////////////////////////////////////////////
void CGingkoToolTip::SetTransparency(BYTE nTransparency /* = 0 */) 
{
	if (nTransparency <= PERCENT_MAX_TRANSPARENCY)
...
////////////////////////////////////////////////////////////////////
// CGingkoToolTip::SetTooltipShadow()
...
////////////////////////////////////////////////////////////////////
void CGingkoToolTip::SetTooltipShadow(int nOffsetX, int nOffsetY, BYTE nDarkenPercent /* = 50 */, 
...
void CGingkoToolTip::SetImageShadow(int nOffsetX, int nOffsetY, BYTE nDarkenPercent /* = 50 */, 
					BOOL bGradient /* = TRUE */, int nDepthX /* = 7 */, int nDepthY /* = 7 */)
{
