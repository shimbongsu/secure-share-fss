...
inline UINT CResizableLayout<T>::AddAnchorCallback(UINT nCallbackID)
{
	...
	LayoutInfo layout;
	if (nCallbackID == -1)
		nCallbackID = (UINT)m_listLayoutCB.GetCount()+1;
	RemoveAnchor(nCallbackID);
	layout.nCallbackID = nCallbackID;
...
inline void CResizableLayout<T>::ArrangeLayout()
{
	...
	size_t count = m_listLayout.GetCount();
	size_t countCB = m_listLayoutCB.GetCount();
	...
	HDWP hdwp = ::BeginDeferWindowPos((int)(count + countCB));
